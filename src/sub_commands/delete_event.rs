...
pub fn delete(
    ...
    let event_id = EventId::from_hex(event_id_to_delete_hex).expect("Invalid event id");

    match client.delete_event(event_id, sub_command_args.reason.clone()) {
        Ok(_) => println!("Deleted event with id: {}", event_id.to_bech32().unwrap()),
        Err(e) => eprintln!("{e}"),
    }
}
